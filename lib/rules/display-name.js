...
module.exports = function(context) {
  ...
  return {
    ...
    ClassDeclaration: function(node) {
      ...
      tern('react.isComponent', context.getFilename(), context.getSourceCode().getText(), node, function(err, isComponent) {
        if (isComponent !== true) {
          return;
        }

        tern('react.componentProperties', context.getFilename(), context.getSourceCode().getText(), node, function(err2, data) {
          var hasDisplayName = data.displayName || (acceptTranspilerName && hasTranspilerName(node));
          if (hasDisplayName) {
